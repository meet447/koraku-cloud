...
export function Navbar() {
  return (
    <nav className="relative z-10 flex w-full items-center justify-between px-6 py-6 md:px-10">
      ...
      <ul className="hidden items-center gap-8 text-sm font-normal text-landing-ink md:flex">
        {NAV_ITEMS.map((item) => (
          <li key={item.label}>
            <Link
              href={item.href}
      ...
      <div className="flex items-center gap-3 md:hidden">
        ...
        <span className="text-xl font-normal tracking-tighter text-landing-ink">Koraku</span>
      </div>
      ...
      <div className="flex flex-1 justify-end">
        <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
          <Link
            href={APP_BASE}
            className="group flex items-center gap-2 rounded-full bg-landing-ink py-1.5 pl-2 pr-4 text-white transition-colors hover:bg-stone-800 md:gap-3 md:py-2 md:pr-6"
          >
